...
contiene_el_tipo = {"debilidad", "resistencias", "inmunidades"}
contiene_los_ataques = {"Tipo","Categoria", "Potencia", "Precision","Pp", "Efecto-Secundario", "Prioridad", "Contacto"}
contienen_los_pokemon = {"Tipo","Ps","Ataque-Fisico","Defensa-Fisico","Ataque-Especial", "Defensa-Especial", "Velocidad","Ataques"}
...
def validar_ataques(ataque:dict, tipos:dict):
    ...
    for nombre_ataque, datos in ataque.items():
        ...
        if not (datos["Categoria"] == "Especial" or datos["Categoria"] == "Fisico"):
            raise ValueError(f"El ataque {nombre_ataque} no es ni fisico ni especial")
        ...
        if 0 > datos["Pp"]:
            raise ValueError(f"El ataque {nombre_ataque} no tiene pps")
        
        if type(datos["Prioridad"]) != int:
            raise ValueError(f"El ataque {nombre_ataque} su prioridad no es un numero")
        if -8 > datos["Prioridad"] > 6:
            raise ValueError(f"El ataque {nombre_ataque} la prioridad esta fuera de rango")
        
        if not (datos["Contacto"] == "Si" or datos["Contacto"] == "No"):
            raise ValueError(f"El ataque {nombre_ataque} debe ser o no de contacto")
...
def validar_pokemon(pokemones:dict, tipos:dict, ataques:dict):
    ...
    for nombre_pokemon, datos in pokemones.items():
        ...
        if datos["Velocidad"] < 0:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener velocidad {datos["Velocidad"]}")
        
        if datos["Ataque-Fisico"] < 1:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener ataque fisico menor a 1")

        if datos["Defensa-Fisico"] < 1:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener defensa fisica menor a 1")
        
        if datos["Ataque-Especial"] < 1:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener ataque especial menor a 1")
        
        if datos["Defensa-Especial"] < 1:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener defensa especial menor a 1")
        
        if datos["Velocidad"] < 1:
            raise ValueError(f"El pokemon {nombre_pokemon} no puede tener velocidad menor a 1")
